...
module.exports = function createDevice(device) {
  ...
  switch (device.model) {
    ...
    case 'switch':
      newDevice = {
        ...
        types: [
          ...
          { identifier: 'button', type: 'button', min: 0, max: 4, sensor: true }
        ]
      };
      break
    case 'motion_sensor':
    ...
    case 'weather':
      newDevice = {
        ...
        types: [
          ...
          { identifier: 'temperature', type: 'temperature', unit: '°C', min: -100, max: 100, sensor: true },
          { identifier: 'humidity', type: 'humidity', unit: '%', min: 0, max: 100, sensor: true },
          { identifier: 'pressure', type: 'pressure', unit: 'Pa', min: 3000, max: 110000, sensor: true }
        ]
      };
      break
    case 'magnet':
    ...
    default:
      ...
      if (!shared.unknownDevices.has(device.sid)) shared.unknownDevices.set(device.sid, device)
  }
  if (newDevice) {

    // *********DEBUG START*******************
    if (shared.debug === 4 || shared.debug === 9) sails.log.debug('Xiaomi createDevice.newDevice :', newDevice)
    // *********DEBUG END*******************
    ...
    return gladys.device.create(newDevice)
      .then((data) => {
        ...
        // *********DEBUG START*******************
        if (shared.debug === 4 || shared.debug === 9) sails.log.debug('Xiaomi createDevice.gladys :', data.device.name + ', id :' + data.device.identifier)
        // *********DEBUG END*******************
